...
The ESP32 polls `/api/magtile` every minute. The saved layout is returned as JSON using normalized `x` and `y` positions, so it can be rendered on either screen orientation.
